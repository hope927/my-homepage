...
| Endpoint | `https://app-dbnycdulzu2p-api-ELbWz8Omem6Y-gateway.appmiaoda.com/rest/2.0/ocr/v1/taxi_receipt` |
| Method | `POST` |
| Content-Type | `application/x-www-form-urlencoded` |
| Auth 模式 | `platform_managed`（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer <INTEGRATIONS_API_KEY>` |
| Third-party Domain | `app-dbnycdulzu2p-api-ELbWz8Omem6Y-gateway.appmiaoda.com` |
...
async function recognizeTaxiReceipt(params: TaxiReceiptParams): Promise<TaxiReceiptResponse> {
  ...
  const response = await fetch(
    "https://app-dbnycdulzu2p-api-ELbWz8Omem6Y-gateway.appmiaoda.com/rest/2.0/ocr/v1/taxi_receipt",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",

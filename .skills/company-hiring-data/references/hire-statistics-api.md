...
| Endpoint | `POST https://app-dbnycdulzu2p-api-zYkZzErqJg4L-gateway.appmiaoda.com/enterprise/hire-statistics` |
| Auth | `platform_managed`，Header: `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/x-www-form-urlencoded` |
| third_part_domain | `app-dbnycdulzu2p-api-zYkZzErqJg4L-gateway.appmiaoda.com` |
...
async function fetchHireStatistics(keyword: string): Promise<HireStatisticsData> {
  const response = await fetch(
    "https://app-dbnycdulzu2p-api-zYkZzErqJg4L-gateway.appmiaoda.com/enterprise/hire-statistics",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",

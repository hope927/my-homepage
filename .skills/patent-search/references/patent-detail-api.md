...
| Endpoint | `https://app-dbnycdulzu2p-api-eLMlJ2jB4oj9-gateway.appmiaoda.com/enterprise/patent/detail` |
| HTTP Method | POST |
| Content-Type | `application/json;charset=UTF-8` |
| Auth 模式 | platform_managed |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| third_part_domain | `app-dbnycdulzu2p-api-eLMlJ2jB4oj9-gateway.appmiaoda.com` |
| 计费 | 未启用（免费接口） |
...
async function fetchPatentDetail(id: string): Promise<PatentDetail> {
  const url = new URL(
    "https://app-dbnycdulzu2p-api-eLMlJ2jB4oj9-gateway.appmiaoda.com/enterprise/patent/detail",
  );

...
| Endpoint | `POST https://app-dbnycdulzu2p-api-rY7JZ6jqr6dL-gateway.appmiaoda.com/v2/ai_search/chat/completions` |
| Content-Type | `application/json` |
| 认证模式 | `platform_managed`（`X-Gateway-Authorization: Bearer <INTEGRATIONS_API_KEY>`） |
| third_part_domain | `app-dbnycdulzu2p-api-rY7JZ6jqr6dL-gateway.appmiaoda.com` |
| 计费 | 计费信息以平台实际配置为准 |
...
async function callBaiduSearch(
  ...
  const response = await fetch(
    "https://app-dbnycdulzu2p-api-rY7JZ6jqr6dL-gateway.appmiaoda.com/v2/ai_search/chat/completions",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",

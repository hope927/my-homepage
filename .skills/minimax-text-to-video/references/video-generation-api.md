...
| 第三方域名 | `app-dbnycdulzu2p-api-V9gDzg15D7BL-gateway.appmiaoda.com` |
...
| 方法 | POST |
| Endpoint | `https://app-dbnycdulzu2p-api-V9gDzg15D7BL-gateway.appmiaoda.com/v1/video_generation` |
...
async function submitTextToVideo(
  ...
  const response = await fetch(
    "https://app-dbnycdulzu2p-api-V9gDzg15D7BL-gateway.appmiaoda.com/v1/video_generation",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
serve(async (req: Request): Promise<Response> => {
  ...
  // --- Call upstream ---
  const upstream = await fetch("https://app-dbnycdulzu2p-api-V9gDzg15D7BL-gateway.appmiaoda.com/v1/video_generation", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "X-Gateway-Authorization": `Bearer ${apiKey}`,
...
| 方法 | GET |
| Endpoint | `https://app-dbnycdulzu2p-api-zYkZz2eDWvPL-gateway.appmiaoda.com/v1/query/video_generation` |
| 计费 | 不计费 |
...
async function queryTextToVideo(taskId: string): Promise<{
  ...
  const url = new URL("https://app-dbnycdulzu2p-api-zYkZz2eDWvPL-gateway.appmiaoda.com/v1/query/video_generation");
...
| 方法 | GET |
| Endpoint | `https://app-dbnycdulzu2p-api-GYX1ld3ExkQa-gateway.appmiaoda.com/v1/files/retrieve` |
| 计费 | 不计费 |
...
async function retrieveVideoFile(fileId: string): Promise<{
  ...
  const url = new URL("https://app-dbnycdulzu2p-api-GYX1ld3ExkQa-gateway.appmiaoda.com/v1/files/retrieve");
